...
function calculateScore(source = "fallback") {
  ...
  currentResult = {
    ...
    source:
      source === "model"
        ? `互动空间离线识别候选：${uploadedImageName || selectedHand.label}`
        : uploadedImageName
          ? `上传图片 + 人工校正：${uploadedImageName}`
          : "稳定样例",
    label: source === "model" ? "离线识别候选牌面" : selectedHand.label,
    tiles,
    fans,
    fan,
    points,
...
async function recognizeWithYolo() {
  ...
  const payload = {
    ok: true,
    model_version: uploadedFile
      ? `互动空间离线 Demo · 已预览 ${uploadedImageName}`
      : "互动空间离线 Demo · 稳定样例",
    tile_count: selectedHand.tiles.length,
    tiles: selectedHand.tiles,
    average_confidence: 0.92,
    detections: selectedHand.tiles.map((tile, index) => ({
      tile,
      class_name: "offline_demo",
      confidence: 0.92,
      center_x: index * 40,
      center_y: 0,
    })),
  ...
  $("#recognitionOutput").innerHTML = `<p class="muted">已生成离线识别候选。互动空间版不调用后端服务，可直接进入算分和账单流程。</p>`;
...
function previewUpload(event) {
  ...
  reader.onload = () => {
    ...
    $("#recognitionOutput").innerHTML = `
      <p class="muted">已上传 ${file.name}。请在“识别结果”中校正牌型，再生成离线识别候选。</p>
    `;
  };
